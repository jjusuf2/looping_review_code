import numpy as np
import pandas as pd
from tqdm.auto import tqdm

import sys
from pathlib import Path
sys.path.append('../')
from utils import *  # key functions for this project

THRESHOLD = 200

dt_arr = np.array([0.02, 0.04, 0.1, 0.2, 0.5, 1, 2, 5, 10, 30, 60, 120])
loop_num_arr = np.array([0, 1, 2, 3, 4, 5])
noise_arr = np.array([0, 10, 20, 50])
temporal_threshold_frames_arr = np.array([3, 6, 10])

n_reps = 19 - 10 + 1  # 10..19 inclusive
total_iters = len(dt_arr) * len(loop_num_arr) * len(noise_arr) * len(temporal_threshold_frames_arr) * n_reps

rows = []

# loop and noise are the outer loops so the 10 trajectories of each (loop, noise) are read from disk once
# and then reused from the load_trajectory cache for every dt and temporal threshold
with tqdm(total=total_iters, desc="calculating times below threshold", unit="rep") as pbar:
    for loop_num in loop_num_arr:
        for noise in noise_arr:
            for dt in dt_arr:
                for temporal_threshold_frames in temporal_threshold_frames_arr:
                    temporal_threshold = temporal_threshold_frames * dt
                    under_threshold_times = under_threshold_times_spatiotemporal_method_all_reps(loop_num, noise, non_sticky=False,
                                                                                                 target_frame_duration=dt,
                                                                                                 threshold=THRESHOLD,
                                                                                                 temporal_threshold=temporal_threshold,
                                                                                                 pbar=pbar)
                    for under_threshold_time in under_threshold_times:
                        rows.append(
                            {
                                "loop": loop_num,
                                "noise": noise,
                                "dt": dt,
                                "threshold": THRESHOLD,
                                "temporal_threshold_frames": temporal_threshold_frames,
                                "time": under_threshold_time,
                            }
                        )

under_threshold_times_df = pd.DataFrame(rows)

under_threshold_times_df.to_csv(f'/mnt/md1/jjusuf/looping_review/analysis_data/all_loops_under_threshold_times_spatiotemporal.csv', index=False, float_format="%.2f")
