...
#     cdef cython.double [:] stallFalloff
    ...
    cdef cython.double [:] longlived_falloff
    ...
    cdef cython.long [:] longlived_SMCs1 #list of where the longlived SMCs reside (left; longlived SMCs2 is right); -1 for unbound SMC
    cdef cython.long [:] longlived_SMCs2
    cdef cython.long [:] longlived_SMCs3 #list of central site if dealing with case of "paired" SMCs
    cdef cython.long [:] stalled1 #list of whether or not each SMC is stalled
    cdef cython.long [:] stalled2
    cdef cython.long [:] longlived_stalled1 #list of whether or not each SMC is stalled
    cdef cython.long [:] longlived_stalled2
    cdef cython.long [:] stalled1_checked #flag signaling if random stalling has been performed
    cdef cython.long [:] stalled2_checked
    cdef cython.long [:] longlived_stalled1_checked #flag signaling if random stalling has been performed
    cdef cython.long [:] longlived_stalled2_checked
    cdef cython.long [:] occupied # list of whether or not site is occupied
#     cdef cython.bool [:] boundaryFlag 
    cdef cython.long [:] DSBFlag 
    cdef cython.long [:] onesided # list of whether or not each SMC is one-sided
    cdef cython.long [:] longlived_onesided # list of whether or not each SMC is one-sided
    ...
    cdef cython.long [:] longlived_inactive_side # list of which of the two sides is inactive. needed to differentiate between two sides in case where active side runs into a stall site
    ...
    cdef cython.double [:] longlived_slidepauseForward # pause probability for diffusing arms (longlived SMCs), so we can regulate v_diffuse / v_active ratio
    cdef cython.double [:] slidepauseBackward
    cdef cython.double [:] longlived_slidepauseBackward
    cdef cython.int [:] sliding_on # used to determine whether safety belt is attached or not
    cdef cython.double [:] belt_off_rate
    cdef cython.double [:] longlived_belt_off_rate
    cdef cython.double [:] belt_on_rate
    cdef cython.double [:] longlived_belt_on_rate
    ...
    cdef cython.double [:] longlived_belt_off_rate_original # since belt_off_rate can be set to 0 sometimes, I need to store this variable for reuse later.
    cdef cython.long [:] belt_attached
    cdef cython.long [:] longlived_belt_attached
    #cdef cython.double [:] slidepauseSum
    cdef cython.double [:] switch
    cdef cython.double [:] longlived_switch
    cdef cython.long [:] switch_life
    cdef cython.long [:] longlived_switch_life
    ...
    def __init__(self, emissionProb, deathProb, longlived_deathProb, stallProbLeft, stallProbRight, pauseProb, 
                 numSmc, longlived_numSmc, onesided,longlived_onesided, paired=0, switch=0, 
                 slide=0, slidepauseForward=0,slidepauseBackward=0, 
                 pushing=0, belt_on=0, belt_off=1., SLIDE_PAUSEPROB=0.9, loop_prefactor=1.5, FULL_LOOP_ENTROPY=1):
        emissionProb[0] = 0
        ...
        self.longlived_SMCs1 = np.zeros((self.longlived_M), int)
        self.longlived_SMCs2 = np.zeros((self.longlived_M), int)
        self.longlived_SMCs3 = np.zeros((self.longlived_M), int)
        self.stalled1 = np.zeros(self.M, int)
        self.stalled2 = np.zeros(self.M, int)
        self.longlived_stalled1 = np.zeros(self.longlived_M, int)
        self.longlived_stalled2 = np.zeros(self.longlived_M, int)
        self.stalled1_checked = np.zeros(self.M, int)
        self.stalled2_checked = np.zeros(self.M, int)
        self.longlived_stalled1_checked = np.zeros(self.longlived_M, int)
        self.longlived_stalled2_checked = np.zeros(self.longlived_M, int)
...
#         self.stallFalloff = stallFalloffProb
        ...
        self.longlived_onesided=onesided
        self.paired=paired
        self.slide=slide
        self.pushing=pushing
        self.belt_attached=np.ones(self.M, int)
        self.longlived_belt_attached=np.ones(self.longlived_M, int)
        ...
        if type(slidepauseForward) in [int,float, np.float64, np.double]: # just in case it's not initialized
            ...
            self.longlived_slidepauseForward=np.zeros(self.longlived_M, np.double)
            for i in range(self.longlived_M):
                self.longlived_slidepauseForward[i]=slidepauseForward
        else:
            self.slidepauseForward=slidepauseForward
            self.longlived_slidepauseForward=slidepauseForward
        if type(slidepauseBackward) in [int,float, np.float64, np.double]: # just in case it's not initialized
            ...
            self.longlived_slidepauseBackward=np.zeros(self.longlived_M, np.double)
            for i in range(self.longlived_M):
                self.longlived_slidepauseBackward[i]=slidepauseBackward
        else:
            self.slidepauseBackward=slidepauseBackward
            self.longlived_slidepauseBackward=slidepauseBackward
        ...
        self.longlived_switch_life=np.zeros(self.longlived_M,long) # time remaining until directional switch
        if type(switch) in [int,float,np.float64,np.double]: # just in case it's not initialized
            ...
            self.longlived_switch=np.zeros(self.longlived_M, np.double)
            for i in range(self.longlived_M):
                self.longlived_switch[i]=switch
        else:
            self.switch=switch
            self.longlived_switch=switch

        if type(belt_on) in [int,float,np.float64,np.double]:
            self.belt_on_rate=np.zeros(self.M, np.double)
            self.longlived_belt_on_rate=np.zeros(self.longlived_M, np.double)
            for i in range(self.M):
                self.belt_on_rate[i] = belt_on
            for i in range(self.longlived_M):
                self.longlived_belt_on_rate[i] = belt_on
        else:
            self.belt_on_rate=belt_on
            self.longlived_belt_on_rate=belt_on
        if type(belt_off) in [int,float,np.float64,np.double]:
            self.belt_off_rate=np.zeros(self.M, np.double)
            self.longlived_belt_off_rate=np.zeros(self.longlived_M, np.double)
            for i in range(self.M):
                self.belt_off_rate[i] = belt_off
                self.belt_off_rate_original[i] = belt_off
            for i in range(self.longlived_M):
                self.longlived_belt_off_rate[i] = belt_off
                self.longlived_belt_off_rate_original[i] = belt_off
        else:
            self.belt_off_rate=belt_off
            self.belt_off_rate_original = belt_off # 
            self.longlived_belt_off_rate=belt_off
            self.longlived_belt_off_rate_original = belt_off # 

        self.inactive_side= np.zeros(self.M,int)
        self.longlived_inactive_side= np.zeros(self.longlived_M,int)
        
        for ind in xrange(self.M):
            self.birth(ind)
            
        for ind in xrange(self.longlived_M):
            self.longlived_birth(ind)
...
#                 continue
    ...
    cdef longlived_birth(self, cython.int ind):
        cdef int pos,i 
...
#                 continue
            ...
            self.longlived_SMCs1[ind] = pos
            self.longlived_SMCs2[ind] = pos+1 #don't let smcs start on single binding site
            if self.paired:
                self.longlived_SMCs3[ind]= pos + np.random.randint(0,2) #just put the center site on one of those two, don't allow shrinking 
            self.occupied[pos] = 1
            self.occupied[pos+1] = 1
            ...
            if self.longlived_onesided[ind]:
                if randnum() < 0.5: #1-sided extrusion - stall 1 arm
                    self.longlived_stalled1[ind]=1
                    self.longlived_inactive_side[ind]=1
                    if self.paired:
                        self.longlived_SMCs3[ind]=self.longlived_SMCs2[ind]
                else:
                    self.longlived_stalled2[ind]=1
                    self.longlived_inactive_side[ind]=2
                    if self.paired:
                        self.longlived_SMCs3[ind]=self.longlived_SMCs1[ind]

                if self.longlived_switch[ind]>0:
                    self.set_longlived_switch_life(ind)

                self.longlived_belt_attached[ind] = 1
                self.longlived_belt_off_rate[ind] = self.longlived_belt_off_rate_original[ind]
    ...
    cdef set_longlived_switch_life(self, cython.int ind):
        self.longlived_switch_life[ind] = int(-np.log(np.random.uniform())/self.longlived_switch[ind])
        return
    ...
    def set_longlived_slidepause(self, slidepauseForward, slidepauseBackward):#cython.double [:] slidepauseForward, cython.double [:] slidepauseBackward):
        self.longlived_slidepauseForward=slidepauseForward
        self.longlived_slidepauseBackward=slidepauseBackward
    ...
    cdef do_longlived_switch(self, cython.int ind):
        self.longlived_inactive_side[ind] = 1 + self.longlived_inactive_side[ind]%2
        # note that this allows repeat attempts against a stall site via directional switching
        if self.longlived_inactive_side[ind] == 1:
            self.longlivedstalled1[ind] = 1
            self.longlivedstalled2[ind] = 0
        else:
            self.longlivedstalled1[ind] = 0
            self.longlivedstalled2[ind] = 1
        self.set_longlived_switch_life(ind)
    ...
    cdef death(self):
        ...
        for i in xrange(self.M):
            ...
            if randnum() < falloff:                 
                ...
                self.stalled1[i] = 0
                self.stalled2[i] = 0
                self.stalled1_checked[i] = 0
                self.stalled2_checked[i] = 0
                
                self.birth(i)
                
        for i in xrange(self.longlived_M):
            falloff1 = self.longlived_falloff[self.longlived_SMCs1[i]]
            falloff2 = self.longlived_falloff[self.longlived_SMCs2[i]]
                    
            falloff = min(falloff1, falloff2)
            if randnum() < falloff:                 
                self.occupied[self.longlived_SMCs1[i]] = 0
                self.occupied[self.longlived_SMCs2[i]] = 0
                if self.paired:
                    self.occupied[self.longlived_SMCs3[i]] = 0
                self.longlived_stalled1[i] = 0
                self.longlived_stalled2[i] = 0
                self.longlived_stalled1_checked[i] = 0
                self.longlived_stalled2_checked[i] = 0
                self.longlived_birth(i)
    ...
    cdef step(self):
        ...
        self.entropic_rates() #set the rates according to the entropy
        ...
        for i in range(self.M):            
            ...
            #stall_site1=0
            #stall_site2=0
            if self.stalled1_checked[i] == 0:
                self.stalled1_checked[i] = 1
                if randnum() < stall1:  # This implementation needs to be changed for stochastic stepping
                    self.stalled1[i] = 1
            ...
            if self.stalled2_checked[i] == 0:
                self.stalled2_checked[i] = 1
                if randnum() < stall2: 
                    self.stalled2[i] = 1
                    #stall_site2=1
            ...
            elif self.slide and (not self.belt_attached[i]) and (self.inactive_side[i]==1):# and (stall_site1==0): #if stall is not due to a stall site, then it is b/c we are looking at inactive side of LEF, which can diffuse if the belt is not attached
        ...
        for i in range(self.longlived_M):            
            stall1 = self.stallLeft[self.longlived_SMCs1[i]]
            stall2 = self.stallRight[self.longlived_SMCs2[i]]

            #these variables will be used to determine whether stall is due to a stall size
            #or simply due to being the inactive side of the extruder
            #stall_site1=0
            #stall_site2=0
            if self.longlived_stalled1_checked[i] == 0:
                self.longlived_stalled1_checked[i] = 1
                if randnum() < stall1: 
                    self.longlived_stalled1[i] = 1
                    #stall_site1=1 # used for book keeping before I had safety belt and inactive_side variables
                    if self.longlived_inactive_side[i]==1:
                        self.longlived_belt_off_rate[i] = 0.
                        self.longlived_belt_attached[i] = 1
            if self.longlived_stalled2_checked[i] == 0:
                self.longlived_stalled2_checked[i] = 1
                if randnum() < stall2: 
                    self.longlived_stalled2[i] = 1
                    #stall_site2=1
                    if self.longlived_inactive_side[i]==2:
                        self.longlived_belt_off_rate[i] = 0.
                        self.longlived_belt_attached[i] = 1

            #note- smc can already be stalled if it is one-sided
     
            cur1 = self.longlived_SMCs1[i]
            cur2 = self.longlived_SMCs2[i]
            
            if self.longlived_stalled1[i] == 0: # not stalled, just go if possible 
                if self.occupied[cur1-1] == 0:
                    pause1 = self.pause[self.longlived_SMCs1[i]]
                    if randnum() > pause1: 
                        self.occupied[cur1-1] = 1
                        self.occupied[cur1] = 0
                        self.longlived_SMCs1[i] = cur1 - 1
                        self.longlived_stalled1_checked[i] = 0
                        if self.paired:#guarantee that center site remains occupied
                            self.occupied[self.longlived_SMCs3[i]] = 1
                elif self.pushing and self.occupied[cur1-1] == 1: # if site occupied, but in a sim where pushing is allowed
                    #first check if pushing is allowed
                    pushing_allowed=1 # start with pushing allowed
                    x = cur1-1 # consider position of LEF of occupied site
                    ...
                    while pushing_allowed and self.occupied[x]: # while we need to check obstacle lefs
                        if x-1<0: # if lef at x can't move, can't push
                            ...
                            y=np.where(np.array(self.longlived_SMCs1)==x)[0] # check SMCs1 to see if it has the leg at x
                            if len(y)>0: # if so...
                                #I think self.stallLeft[x]<0.99999 should be changed to not stall_site1, or else, maybe stallLeft<1e-8... not sure why I didn't just use stallSite.
                                #didn't use stall_site, because I didn't want to have to have randnum() < stall.  I want stalled & finite stall prob.
                                if self.longlived_stalled1[y[0]] and (not (self.longlived_[x]>0.0)) and (not self.longlived_belt_attached[y[0]]): #check if stalled (inactive) and not due to stallLeft=1 (not sure what cython does with == for floats), this allows pushing over real stalls with stall prob <1, but i'm currently only using stall to identify inactive sites and the end of the chain. 
                                    x=x-1 # move on to check next position
                                else:# smc at x is an active leg ..break
                                    pushing_allowed=0
                                    break
                            else: # otherwise, check SMCs2
                                y=np.where(np.array(self.longlived_SMCs2)==x)[0]
                                #len y should necessarily be >0 (since occupied is a condition to be in the while loop)
                                if self.longlived_stalled2[y[0]] and (not (self.stallRight[x]>0.0)) and (not (self.stallLeft[x]>0.0)) and not self.longlived_belt_attached[y[0]]: # SMCs2 move rightward, so don't really care if stallLeft==1... but do want to make sure I'm not pushing an active leg. and don't want to push past a stall size
                                    x=x-1
                                else:
                                    pushing_allowed=0
                                    break
                    if pushing_allowed:
                        ...
                        if randnum() > pause1:
                            ...
                            x=x+1
                            while self.occupied[x] == 1 and not x==cur1: # go through chain, moving SMCs, note that self.occupied does not change except ends
                                y=np.where(np.array(self.longlived_SMCs1)==x)[0]
                                if len(y)>0:
                                    self.longlived_SMCs1[y[0]]=x-1
                                else:
                                    y=np.where(np.array(self.longlived_SMCs2)==x)[0]
                                    self.longlived_SMCs2[y[0]]=x-1
                                x=x+1
                            self.occupied[cur1]=0
                            self.longlived_SMCs1[i]=cur1-1
            ...
            elif self.slide and (not self.longlived_belt_attached[i]) and (self.longlived_inactive_side[i]==1):# and (stall_site1==0): #if stall is not due to a stall site, then it is b/c we are looking at inactive side of LEF, which can diffuse if the belt is not attached
                #HERE IS WHERE I MAKE CHANGES AND PUT IN slidepauseforward and slidepausebackward
                #inactive side 1, slide pause forward goes with cur-1, i.e., first if
                #inactive side 2, slide pause forward goes with cur+1 i.e., second if
                #so if-else-if scheme might need to be changed
                if (self.occupied[cur1-1]==0) and (self.occupied[cur1+1]==0):
                    rr=randnum()
                    if rr < 2.-self.longlived_slidepauseForward[i]-self.longlived_slidepauseBackward[i]:
                        #probability of sliding one way or the other
                        if rr < 1.-self.longlived_slidepauseForward[i]:
                            #slide forward
                            self.occupied[cur1-1]=1
                            self.longlived_SMCs1[i]=cur1-1
                        else:
                    ...
                    if randnum() > self.longlived_slidepauseForward[i]:
                        #slide forward
                        self.occupied[cur1-1]=1
                        self.longlived_SMCs1[i]=cur1-1
                        self.occupied[cur1]=0
                    ...
                    if randnum() > self.longlived_slidepauseBackward[i]:
                        #slide backward
                        self.occupied[cur1+1]=1
                        self.longlived_SMCs1[i]=cur1+1
                        self.occupied[cur1]=0
            ...
            if self.longlived_stalled2[i] == 0:
                if self.occupied[cur2 + 1] == 0:
                    pause2 = self.pause[self.longlived_SMCs2[i]]
                    if randnum() > pause2: 
                        self.occupied[cur2 + 1] = 1
                        self.occupied[cur2] = 0
                        self.longlived_SMCs2[i] = cur2 + 1
                        self.longlived_stalled2_checked[i] = 0
                        if self.paired:#guarantee that center site remains occupied
                            self.occupied[self.longlived_SMCs3[i]] = 1
                elif self.pushing and self.occupied[cur2+1] == 1: # if site occupied, but in a push sim
                    ...
                    while pushing_allowed and self.occupied[x]: # while we need to check obstacle lefs
                        if x+1>=self.N: # if lef at x can't move, can't push
                            ...
                            y=np.where(np.array(self.longlived_SMCs1)==x)[0] # check SMCs1 to see if it has the leg at x
                            if len(y)>0: # if so...
                                if self.longlived_stalled1[y[0]] and (not (self.stallRight[x]>0.0)) and (not (self.stallLeft[x]>0.0)) and not self.longlived_belt_attached[y[0]]: #check if stalled (inactive) and not due to stall site, verify not active
                                    x=x+1 # move on to check next position
                                else:# smc at x is an active leg ..break
                                    pushing_allowed=0
                                    break
                                ...
                                y=np.where(np.array(self.longlived_SMCs2)==x)[0]
                                #len y should necessarily be >0 (since occupied is a condition to be in the while loop)
                                if self.longlived_stalled2[y[0]] and (not (self.stallRight[x]>0.0)) and not self.belt_attached[y[0]]: # SMCs2 move rightward, so don't really care if stallLeft==1... but do want to make sure I'm not pushing an active leg. and don't want to push past a stall size
                                    x=x+1
                                else:
                                    pushing_allowed=0
                                    break
            ...
            elif self.slide and (not self.longlived_belt_attached[i]) and (self.longlived_inactive_side[i]==2):# and (stall_site2==0):
                if (self.occupied[cur2-1]==0) and (self.occupied[cur2+1]==0):
                    rr=randnum()
                    #print(rr)
                    #ratesum = self.slidepauseForward[i]
                    #ratesum += self.slidepauseBackward[i]
                    if rr < 2.-self.longlived_slidepauseForward[i]-self.longlived_slidepauseBackward[i]: #ratesum:
                        #probability of sliding one way or the other
                        if rr < 1.-self.longlived_slidepauseForward[i]:
                            #slide forward
                            self.occupied[cur2+1]=1
            ...
            if self.longlived_switch[i]>0:
                if self.longlived_switch_life[i] == 0:
                    self.do_longlived_switch(i)
                else:
                    self.longlived_switch_life[i] -= 1

            if self.longlived_belt_attached[i]:
                if randnum() < self.longlived_belt_off_rate[i]:
                    self.longlived_belt_attached[i]=0
            else:
                if randnum() < self.longlived_belt_on_rate[i]:
                    self.longlived_belt_attached[i]=1
    ...
    def getlonglivedSMCs(self):
        if np.any(self.onesided):
            return np.array(self.longlived_SMCs1), np.array(self.longlived_SMCs2), np.array(self.longlived_inactive_side)
        elif not self.paired:
            return np.array(self.longlived_SMCs1), np.array(self.longlived_SMCs2)
        else:
            return np.array(self.longlived_SMCs1), np.array(self.longlived_SMCs2), np.array(self.longlived_SMCs3)
    ...
    def updateMap(self, cmap):
        ...
        cmap[self.longlived_SMCs1, self.longlived_SMCs2] += 1
        cmap[self.longlived_SMCs2, self.longlived_SMCs1] += 1

    def updatePos(self, pos, ind):
        pos[ind, self.SMCs1] = 1
        pos[ind, self.SMCs2] = 1
        
    def updatelonlivedPos(self, pos, ind):
        pos[ind, self.longlived_SMCs1] = 1
        pos[ind, self.longlived_SMCs2] = 1


    def entropic_rates(self):
        if np.any(self.onesided):
            left, right, inactive = self.getSMCs() #SMCTran.getSMCs()
        elif not self.paired:
            left, right = self.getSMCs()
        else:
            left, right, center = self.getSMCs()
    ...
    def longlived_entropic_rates(self):
        if np.any(self.onesided):
            left, right, inactive = self.getlonglivedSMCs() #SMCTran.getSMCs()
        elif not self.paired:
            left, right = self.getlonglivedSMCs()
        else:
            left, right, center = self.longlived_getSMCs()
        lefsites= [[left[k],k] for k in range(len(left))]
        lefsites.extend([[right[k],k] for k in range(len(right))])
        # allBonds.append(bonds)
        ...
        #i=bonds[0]#min(left) # 0 skip some empty sites.
        #lef_list=[]
        loop_len=np.zeros(self.longlived_M)
        parent_loop = np.zeros(self.longlived_M,dtype=np.int64)-1 # i^th component is the index of the smc that contains smc i. or -1 if none.
        loop_entropy_forward=np.zeros(self.longlived_M,dtype=np.float64)
        loop_entropy_backward=np.zeros(self.longlived_M,dtype=np.float64)
        ...
        #adjust rates based on loop_entropy
        slidePauseArrayForward=np.zeros(self.longlived_M) + 1.-(1.-self.SLIDE_PAUSEP)*np.exp(loop_entropy_forward)
        slidePauseArrayBackward=np.zeros(self.longlived_M) + 1.-(1.-self.SLIDE_PAUSEP)*np.exp(loop_entropy_backward)

        #feed in new rates to smctranslocator
        #slidePauseSum=slidePauseArrayForward+slidePauseArrayBackward
        #SMCTran
        self.set_longlived_slidepause(slidePauseArrayForward, slidePauseArrayBackward)#, slidePauseSum)
    ...
    def updateFalloffProb(self, falloffProb, falloffProbSuper):
        ...
        self.longlived_falloff = falloffProbSuper
    ...
    def updatePauseProb(self, PauseProb):
        self.pause = PauseProb
        
    def get_stall_prob(self):
        return np.array(self.stallLeft), np.array(self.stallRight)
    
    def get_falloff_prob(self):
        return np.array(self.falloff), np.array(self.longlived_falloff)
    
    def get_emission(self):
        return np.array(self.emission)