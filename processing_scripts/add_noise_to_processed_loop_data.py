...
noise_um = 0
for loop_num in [0, 1, 2, 3, 4, 5, 6, 7, 8]:
    for rep in range(10,20):
        mat = np.loadtxt(f'../simulation_data_processed/loop_{loop_num}_{rep}_step_{step}.txt')
        ...
        np.savetxt(f'../simulation_data_processed/loop_{loop_num}_{rep}_step_{step}_noise_{noise_um}um.txt', mat_with_dist_3D, fmt='%.2f')
